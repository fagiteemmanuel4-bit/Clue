...
async function requestNonStreaming(baseUrl: string, apiKey: string, model: string, messages: ChatMessage[], signal?: AbortSignal) {
  ...
  const response = await fetch(`${baseUrl.replace(/\/$/, '')}/chat/completions`, {
    ...
    headers: {
      ...
      ...(isOpenRouter ? {'HTTP-Referer': process.env.NEXT_PUBLIC_APP_URL || 'https://clue-nmmn.vercel.app','X-Title':'Clue'} : {}),
    },
    body: JSON.stringify({
      model, messages, temperature: 0.4, max_tokens: 900, stream: false,
      ...(isOpenRouter ? { tools: [{ type: 'openrouter:web_search', parameters: { engine: 'auto', max_results: 5 } }] } : {}),
    }),
  })
  if (!response.ok) {
  ...
  const sources = (choice?.annotations || [])
    .map(a => a.url_citation)
    .filter((x): x is {url:string;title?:string} => Boolean(x?.url))
...
function openAICompatible(id: string, label: string, baseUrl: string, apiKey: string, model: string, fallbackModels: string[] = []): AIProvider {
  return { id, label, async stream(messages, signal) {
    ...
    for (const candidate of candidates) {
      try { return textStream(await requestNonStreaming(baseUrl, apiKey, candidate, messages, signal)) }
