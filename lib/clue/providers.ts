...
async function requestNonStreaming(baseUrl: string, apiKey: string, model: string, messages: ChatMessage[], signal?: AbortSignal, maxTokens = 900) {
  ...
  const sources = uniqueSources((choice?.annotations || []).map(a => a.url_citation).filter((x): x is {url:string;title?:string} => Boolean(x?.url)))
  content += sourceMarker(sources)
    ...
    async pull(outController) {
      try {
        while (true) {
          ...
          if (done) { buffer += decoder.decode(); if (buffer.trim()) processSSE(buffer); const unique=uniqueSources(sources); if(unique.length) outController.enqueue(encoder.encode(sourceMarker(unique))); closed=true; outController.close(); return }
          buffer += decoder.decode(value,{stream:true}); const events=buffer.split(/\r?\n\r?\n/); buffer=events.pop()||''
          for (const event of events) {
            const doneEvent=processSSE(event); if(doneEvent){const unique=uniqueSources(sources);if(unique.length)outController.enqueue(encoder.encode(sourceMarker(unique)));closed=true;outController.close();await reader.cancel();return}
            const delta=extractDelta(event); if(delta){outController.enqueue(encoder.encode(delta));return}
          }
        }
      } catch(error){closed=true;if((error as Error)?.name==='AbortError'||combined.aborted){outController.error(error);return}outController.error(error)}
    },
    cancel(){closed=true;return reader.cancel()},
  })
  function extractDelta(event:string){
    const dataLines=event.split(/\r?\n/).filter(line=>line.startsWith('data:')).map(line=>line.slice(5).trimStart())
    for(const data of dataLines){if(!data||data==='[DONE]')continue;try{const json=JSON.parse(data) as {choices?:Array<{delta?:{content?:string};message?:{content?:string};annotations?:Array<{url_citation?:{url?:string;title?:string}}>}>};const annotations=json.choices?.[0]?.annotations||[];sources.push(...annotations.map(a=>a.url_citation).filter((x):x is {url:string;title?:string}=>Boolean(x?.url)));const delta=json.choices?.[0]?.delta?.content??json.choices?.[0]?.message?.content;if(typeof delta==='string'&&delta)return delta}catch{}}
    return ''
  }
  function processSSE(event:string){for(const data of event.split(/\r?\n/).filter(line=>line.startsWith('data:')).map(line=>line.slice(5).trimStart()))if(data==='[DONE]')return true;return false}
