...
function extractJson(value: string): Composed {
  const cleaned = value.replace(/^```(?:json)?\s*/i, '').replace(/\s*```$/i, '').trim()
  const start = cleaned.indexOf('{'); const end = cleaned.lastIndexOf('}')
  if (start < 0 || end <= start) throw new Error('AI returned invalid file content.')
  return JSON.parse(cleaned.slice(start, end + 1)) as Composed
}

async function composeWithAI(intent: FileIntent): Promise<Composed> {
  const key = process.env.OPENROUTER_API_KEY?.trim() || process.env.AI_API_KEY?.trim()
  if (!key) throw new Error('No AI provider is configured.')
  const base = (process.env.AI_BASE_URL?.trim() || 'https://openrouter.ai/api/v1').replace(/\/$/, '')
  const model = process.env.AI_MODEL?.trim() || 'openrouter/free'
  const schema = intent.format === 'xlsx'
    ? '{"title":"string","rows":[["cell","cell"]]}'
    : intent.format === 'pptx'
      ? '{"title":"string","slides":[{"title":"string","text":"string"}]}'
      : intent.format === 'zip'
        ? '{"title":"string","files":[{"name":"README.md","content":"string"}]}'
        : '{"title":"string","text":"string"}'
  const system = `You are Clue's professional document production engine. Turn the user's request into COMPLETE, useful, polished file content. Never output code, implementation instructions, placeholders, or boilerplate unless the user explicitly requested code. For Word/PDF, write the actual document with headings, sections, lists and professional wording as appropriate. For spreadsheets, create a real structured dataset/table with a header row, meaningful rows, and useful columns; never return spreadsheet code or markdown. For presentations, create coherent slides with concise speaker-ready content. Return ONLY valid JSON matching this shape: ${schema}`
  const response = await fetch(`${base}/chat/completions`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${key}`, 'HTTP-Referer': process.env.NEXT_PUBLIC_APP_URL || 'https://clue-nmmn.vercel.app', 'X-Title': 'Clue' },
    body: JSON.stringify({ model, temperature: 0.35, max_tokens: 6000, stream: false, messages: [{ role: 'system', content: system }, { role: 'user', content: `Create the file requested below. Preserve every important requirement and make reasonable professional choices without asking follow-up questions.\n\n${intent.request}` }] })
  })
  if (!response.ok) throw new Error(`AI file composition failed (${response.status}).`)
  const json = await response.json() as { choices?: Array<{ message?: { content?: string } }> }
  const content = json.choices?.[0]?.message?.content
  if (!content?.trim()) throw new Error('AI returned empty file content.')
