...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) { try { const user = await requireUser(); const p = parseJson(paramsSchema, await params); if (!p.ok) return p.response; const b = parseJson(bodySchema, await request.json().catch(() => null)); if (!b.ok) return b.response; const [current] = await owned(p.data.id, user.id); if (!current) return Response.json({ error: 'Not found' }, { status: 404 }); let title = b.data.title; if (title && current.title !== 'New conversation') { const [firstUser] = await db.select({ content: messages.contentText }).from(messages).where(and(eq(messages.conversationId, p.data.id), eq(messages.role, 'user'))).orderBy(asc(messages.createdAt)).limit(1); const autoCandidate = firstUser?.content.slice(0, 48).trim(); if (autoCandidate && title === autoCandidate) title = undefined }
  const [row] = await db.update(conversations).set({ ...(title !== undefined ? { title } : {}), updatedAt: new Date() }).where(and(eq(conversations.id, p.data.id), eq(conversations.userId, user.id))).returning(); return row ? Response.json({ conversation: row }) : Response.json({ error: 'Not found' }, { status: 404 }) } catch { return Response.json({ error: 'Unauthorized' }, { status: 401 }) } }
